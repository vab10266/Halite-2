...
public class GameMap {
    ...
    public ArrayList<Entity> objectsBetween(Position start, Position target, ArrayList<Entity> list) {
        final ArrayList<Entity> entitiesFound = new ArrayList<>();

        addEntitiesBetween(entitiesFound, start, target, planets.values());
        addEntitiesBetween(entitiesFound, start, target, allShips);
        addEntitiesBetween(entitiesFound, start, target, list);
        return entitiesFound;
    }
    ...
    public TreeMap<Double, Ship> nearbyShipsByDistance(final Entity entity) {
        final TreeMap<Double, Ship> shipByDistance = new TreeMap<>();

        for (final Ship ship : allShips) {
            if (ship.equals(entity)) {
                continue;
            }
            shipByDistance.put(entity.getDistanceTo(ship), ship);
        }

        return shipByDistance;
    }

    public TreeMap<Double, Planet> nearbyPlanetsByDistance(final Entity entity) {
        ...
        for (final Planet planet : planets.values()) {
            ...
            planetByDistance.put(entity.getDistanceTo(planet), planet);
        }
